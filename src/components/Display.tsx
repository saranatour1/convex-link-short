...
export const Display = () => {
  ...
  const user = useQuery(api.links.viewer);
  return (
    <ScrollArea className="flex flex-col h-[40rem] items-start justify-center gap-y-4">
      {user && viewData?.map((item) => <ShortLink key={item._id} item={item} />)}
    </ScrollArea>
  );
};
...
export const CopyButton = ({ children, text, ...props }: CopyButtonProps) => {
  ...
  return (
    <Button {...props} variant={`ghost`} onClick={copyMe}>
